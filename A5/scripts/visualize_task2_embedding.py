...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize Task 2 embeddings with a 2D scatter plot.")
    parser.add_argument("--root", type=str, default="data", help="Path to CIFAR-10 data directory.")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=str(PROJECT_ROOT / "reports" / "task2" / "data" / "task2_cnn_autoencoder.pt"),
        help="Path to the Task 2 checkpoint.",
    )
...
def plot_embedding_scatter(
    ...
    ax.set_title("Task 2: 800 CIFAR-10 samples projected to 2D (PCA of 4D latent space)")
    ax.set_xlabel("PC 1")
    ax.set_ylabel("PC 2")
...
def main() -> None:
    ...
    output_root = PROJECT_ROOT / "reports" / "task2"
    figures_dir = output_root / "figures"
    data_dir = output_root / "data"
    figures_dir.mkdir(parents=True, exist_ok=True)
    data_dir.mkdir(parents=True, exist_ok=True)

    scatter_path = figures_dir / "task2_embedding_scatter.png"
    plot_embedding_scatter(scatter_path, coordinates_2d, labels, CIFAR10_CLASSES)

    np.savez_compressed(
        data_dir / "task2_embedding_scatter.npz",
    ...
    metadata_path = data_dir / "task2_embedding_scatter.json"
    ...
    print(f"saved_data={data_dir / 'task2_embedding_scatter.npz'}")
