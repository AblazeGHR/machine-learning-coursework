...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Task 2 CNN Autoencoder on CIFAR-10.")
...
def save_reconstruction_preview(model: nn.Module, loader, device: torch.device, output_path: Path) -> None:
    ...
    save_image_grid(
        ...
        title="Task 2: original vs reconstruction",
        nrow=8,
...
def save_loss_curve(history: list[dict], output_path: Path) -> None:
    ...
    ax.set_title("Task 2: Loss curve")
...
def main() -> None:
    ...
    model = CNNAutoencoder(latent_dim=args.latent_dim).to(device)
    ...
    ckpt_path = data_dir / "task2_cnn_autoencoder.pt"
    torch.save(checkpoint, ckpt_path)

    history_path = data_dir / "task2_history.json"
    history_path.write_text(json.dumps(history, indent=2), encoding="utf-8")

    preview_path = figures_dir / "task2_reconstruction_preview.png"
    save_reconstruction_preview(model, test_loader, device, preview_path)

    loss_curve_path = figures_dir / "task2_loss_curve.png"
